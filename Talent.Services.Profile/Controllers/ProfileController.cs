...
namespace Talent.Services.Profile.Controllers
{
    ...
    public class ProfileController : Controller
    {
        ...
        [Authorize(AuthenticationSchemes = JwtBearerDefaults.AuthenticationScheme, Roles = "recruiter,employer")]
        public async Task<IActionResult> GetTalentListAsync(FeedIncrementModel feed)
        {
            try
            {
                var result = await _profileService.GetFullTalentList(feed.Position, feed.Number);
                return Json(new { Success = true, Data = result });
            }
